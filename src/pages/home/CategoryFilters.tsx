...
type Props = {
    ...
    priceFilter: { mode: string; below?: number; above?: number; min?: number; max?: number };
...
const CategoryFilters = ({
    ...
    const activeFilters: Array<{ label: string; icon: string; onClear?: () => void }> = [];

    if (selectedCategoryName) {
        activeFilters.push({ label: selectedCategoryName, icon: "/cancel.svg", onClear: handleBackToHome });
    }
    if (selectedLocation) {
        activeFilters.push({ label: selectedLocation, icon: "/location.svg", onClear: () => setSelectedLocation(null) });
    }
    if (selectedTimeframe !== "anytime") {
        activeFilters.push({ label: timeframeLabels[selectedTimeframe], icon: "/time-svgrepo-com.svg", onClear: () => setSelectedTimeframe("anytime") });
    }
    if (priceSort !== "none") {
        activeFilters.push({ label: priceSortLabels[priceSort], icon: "/price-tag-svgrepo-com.svg", onClear: () => setPriceSort("none") });
    }
    if (timeframeSort !== "none") {
        activeFilters.push({ label: dateSortLabels[timeframeSort], icon: "/date-range-svgrepo-com.svg", onClear: () => setTimeframeSort("none") });
    }
    const priceFilterLabel = getPriceFilterLabel();
    if (priceFilterLabel) activeFilters.push({ label: priceFilterLabel, icon: "/filter-svgrepo.svg", onClear: () => setPriceFilter({ mode: "none" }) });
    ...
    return (
        <div className="flex flex-wrap justify-center sm:justify-center gap-2 sm:gap-4 mt-4 mb-6 px-3 sm:px-0">
            {activeFilters.map((filter, idx) => (
                <button key={idx} onClick={filter.onClear} className="bg-white flex-1 sm:flex-none min-w-[140px] max-w-[200px] px-3 sm:px-4 py-2 rounded-lg text-sm flex items-center justify-center gap-2 hover:bg-gray-100 transition-colors">
                    {filter.label}
