...
const ScrollableAds = ({ categories, productsByCategory, applyFilters, productsLoading, handleArrowClick, handleAdClick }: Props) => {
    ...
    return <>{categoriesWithAds.map((category) => (
        <ScrollableAdRow key={category.id} category={category} productsByCategory={productsByCategory} applyFilters={applyFilters} productsLoading={productsLoading} handleArrowClick={handleArrowClick} handleAdClick={handleAdClick} />
    ))}</>;
};
