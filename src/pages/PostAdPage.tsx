...
function PostAdForm() {
  ...
  const [daily, setDaily] = useState("Duration");
  const [weekly, setWeekly] = useState("Duration");
  const [monthly, setMonthly] = useState("Duration");

function handleSelect(trigger: string, value: string) {
  if (trigger === "daily") setDaily(value);
  if (trigger === "weekly") setWeekly(value);
  if (trigger === "monthly") setMonthly(value);
  console.log(`${trigger} chosen:`, value);
}
      ...
      <div className="text-xs flex flex-col lg:flex-row w-full h-full overflow-y-autogap-6 lg:gap-2 py-3 pr-2">   
        <div className="flex flex-col w-full lg:w-3/5 shadow-lg bg-white rounded-xl p-6 space-y-4 lg:overflow-y-scroll no-scrollbar">
          <div className="grid grid-cols-1 gap-2">
            <div>
              ...
              <DropdownPopup
                ...
                options={["Electronics", "Furniture", "Vehicles", "Clothing", "Books"]}
                onSelect={(opt) => {
                  setCategory(opt)
          ...
          <div>
            ...
            <div className="flex gap-3">
              {["Sale", "Pay Later", "Rent"].map((option) => (
                <label
                  key={option}
                  className="relative flex items-center gap-1 bg-[var(--div-active)] rounded-lg px-4 py-2 pt-3.5 pr-4 cursor-pointer"
                >
                  <input type="radio" name="purpose" className="absolute top-1 right-0.5 accent-[var(--dark-def)]"/>
                  <span>{option}</span>
                </label>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {[
              ...
              <div key={label.trigger}>
                ...
                <div className="relative grid gap-2" style={{ gridTemplateColumns: "2fr 1fr" }}>
                  <input
                    type="number"
                    placeholder="0"
          ...
          <div>
            <div>
              <select className="w-full p-3 border rounded-xl border-[var(--div-border)] text-gray-400">
                <option>Ad Area Location</option>
              </select>
            </div>
          ...
          <div>
            <div>
              <select className="w-full p-3 border rounded-xl border-[var(--div-border)] text-gray-400">
                <option>Ad Actual Map Location</option>
              </select>
            </div>
            <div className="flex flex-wrap gap-1 ">
              {["Home Spintex", "Shop Accra", "Shop East Legon", "Shop Kumasi"].map(
                (loc) => (
                  <button
                    key={loc}
                    className="p-1 bg-[var(--div-active)] font-bold mt-1 rounded-xs text-[length:0.9vw] hover:bg-gray-200"
                  >
                    {loc}
                  </button>
                )
        ...
        <div className="relative flex flex-col w-full lg:w-2/5 bg-white shadow-lg rounded-xl p-6">
          ...
          <div className="flex justify-between text-xs scale-60 -ml-17 text-gray-500 my-1 gap-1">
            <span className="whitespace-nowrap py-1 px-2 bg-[var(--div-active)] rounded-xl">{uploadedImages.length} images added</span>
            <span className="whitespace-nowrap py-1 px-2 bg-[var(--div-active)] rounded-xl">• Drag images to arrange</span>
            <span className="whitespace-nowrap py-1 px-2 bg-[var(--div-active)] rounded-xl">• Tap image twice to delete</span>
          </div>
          ...
          <div className="lg:absolute lg:-ml-5 bottom-4 w-full flex justify-center items-center">
            <button className="w-[80%] hover:bg-[var(--accent)] hover:border border-[var(--div-border)] rounded-2xl py-3 text-center cursor-pointer bg-[var(--div-active)]">
              Save
            </button>
          </div>
        </div>
