...
const SubscriptionPage = () => {
  ...
  const handleSubscribe = (id: number) => {
    (async () => {
      try {
        setSubscribingId(id);
        const callbackUrl = `${window.location.origin}/paystack/callback?subscription_id=${encodeURIComponent(
          String(id),
        )}`;
  ...
  const handleRenew = (userSubId: number, subscriptionId: number) => {
    (async () => {
      try {
        ...
        const callbackUrl = `${window.location.origin}/paystack/callback?user_subscription_id=${encodeURIComponent(
          String(userSubId),
