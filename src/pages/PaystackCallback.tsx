...
const PaystackCallback = () => {
    ...
    useEffect(() => {
        (async () => {
            ...
            const userSubId = searchParams.get("user_subscription_id") || searchParams.get("user_subscription") || searchParams.get("userSubscriptionId") || searchParams.get("id");
            ...
            if (userSubId && pendingObj?.subscription_id) {
                ...
                updateUserSub.mutate(
                    { id: Number(userSubId), body: { subscription_id: pendingObj.subscription_id } },
                    {
                        onSuccess: () => {
            ...
            if (pendingObj?.subscription_id) {
                ...
                for (let i = 0; i < maxAttempts; i++) {
                    try {
                        ...
                        found = list.find((us: any) => us.subscription?.id === pendingObj!.subscription_id) ?? null;
                        if (found) break;
                    } catch (e) {
                        // ignore and retry
                    }
                ...
                if (found) {
                    ...
                    updateUserSub.mutate(
                        { id: Number(found.id), body: { subscription_id: pendingObj.subscription_id } },
                        {
                            onSuccess: () => {
                                localStorage.removeItem("pending_subscription");
                                qc.invalidateQueries({ queryKey: ["user-subscriptions"] });
