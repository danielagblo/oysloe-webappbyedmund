...
export default function useAccountDeleteRequest() {
  ...
  const mutation = useMutation<AccountDeleteRequest, Error, { reason: string }, unknown>({
    mutationFn: (variables: { reason: string }) => createAccountDeleteRequest(variables),
    onSuccess: () => {
  ...
  const { mutateAsync, isPending, isError, error, data } =
    mutation as UseMutationResult<AccountDeleteRequest, Error, { reason: string }, unknown>;
