...
import chatService, { type ChatRoom, type Message } from "../../services/chatService";
...
export type UseWsChatReturn = {
  ...
  sendMessage: (roomId: string, text: string, tempId?: string, file?: File | Blob) => Promise<void>;
...
export default function useWsChat(): UseWsChatReturn {
  ...
  const ensureRoomClient = useCallback(async (roomId: string) => {
    ...
    console.log("useWsChat.ensureRoomClient: creating WebSocketClient", { url, tokenPresent: !!token, roomId });
    ...
    const createClient = (attemptUrl: string) =>
      new WebSocketClient(attemptUrl, token, {
        ...
        onClose: (ev) => {
          console.log("useWsChat: socket closed", { roomId, code: ev?.code, reason: ev?.reason, url: attemptUrl });
          // If server closed immediately (1006) and we haven't tried the alt URL, try it once
          if ((ev?.code === 1006 || ev?.code === 1005) && !triedAlternative && attemptUrl !== altUrl) {
            triedAlternative = true;
            console.log("useWsChat: retrying with alternative URL", { altUrl, roomId });
            ...
            try {
              ...
              console.warn("useWsChat: alt client connect failed", err, { roomId, altUrl });
            }
          }
        },
        onError: (ev) => {
          console.warn("useWsChat: socket error", { roomId, ev, url: attemptUrl });
        },
        onMessage: (data) => {
          if (!data) return;
          ...
          if ((data as any).id) {
            ...
            setMessages((prev) => {
              ...
              // dedupe by id
              if (list.find((m) => String(m.id) === String(incoming.id))) return prev;

              // If server echoed our temp_id, replace the optimistic placeholder
              const tempId = (incoming as any).temp_id;
              if (tempId) {
                const byTemp = list.findIndex((m) => (m as any).__temp_id === tempId);
                if (byTemp !== -1) {
                  const newList = [...list];
              ...
              const likelyIndex = list.findIndex((m) => {
                ...
                if (m.sender?.id && incoming.sender?.id && m.sender.id !== incoming.sender.id) return false;
  ...
  const connectToTempChat = useCallback(async (email: string) => {
      ...
      onMessage: (data) => {
        ...
        if (resolvedRoom) {
          ...
          if ((data as any).messages) {
            setMessages((prev) => ({ ...prev, [rid]: (data as any).messages as Message[] }));
          }
        ...
        if ((data as any).id && (data as any).room) {
          ...
          setMessages((prev) => ({ ...prev, [rid]: [...(prev[rid] || []), data as Message] }));
        }
      },
    });
    ...
    const client = new WebSocketClient(url, token, {
      onMessage: (data) => {
        ...
        if ((data as any).room_id || (data as any).id) {
          ...
          setChatrooms((prev) => {
            const found = prev.find((r) => String(r.id) === String(updated.id) || r.room_id === (updated as any).room_id);
  ...
  const sendMessage = useCallback(async (roomId: string, text: string, tempId?: string, file?: File | Blob) => {
    ...
    if (file) {
      ...
      const tryEmbedAsDataUrl = async () => {
        try {
          ...
          if (res && (res as any).id) {
            ...
            if (returnedTemp && !(msg as any).temp_id) (msg as any).temp_id = returnedTemp;
            ...
            const possibleUrl =
              asAny.content && String(asAny.content).startsWith("http")
                ...
                : asAny.image_url || asAny.file_url || asAny.image || (asAny.attachments && asAny.attachments[0] && (asAny.attachments[0].url || asAny.attachments[0].file)) || null;
            if (!asAny.content && possibleUrl) asAny.content = String(possibleUrl);

            setMessages((prev) => {
              const list = prev[roomId] || [];
              if (returnedTemp) {
                const idx = list.findIndex((m) => ((m as any).__temp_id || (m as any).temp_id) === returnedTemp);
                if (idx !== -1) {
                  const newList = [...list];
                  newList[idx] = msg;
                  return { ...prev, [roomId]: newList };
          ...
          console.debug("useWsChat: embed dataUrl send failed, falling back to multipart", err);
        }
        return false;
      };
      ...
      try {
        ...
        // log for debugging
        try {
          console.debug("useWsChat: uploading file", { roomId, tempId, fileName: (file as any).name });
        } catch {}
        const res = await chatService.sendMessageToRoom(roomId, form as unknown as any);
        if (res && (res as any).id) {
          const msg = res as Message & { temp_id?: string };
    ...
    try {
      ...
      if (res && (res as any).id) {
        ...
        setMessages((prev) => {
          ...
          if (returnedTemp) {
            const idx = list.findIndex((m) => ((m as any).__temp_id || (m as any).tempId || (m as any).temp_id) === returnedTemp);
            if (idx !== -1) {
              const newList = [...list];
              newList[idx] = msg;
              return { ...prev, [roomId]: newList };
          ...
          const likelyIndex = list.findIndex((m) => {
            if (!m) return false;
            if (m.sender?.id && msg.sender?.id && m.sender.id !== msg.sender.id) return false;
            if (m.content !== msg.content) return false;
            const mTime = m.created_at ? new Date(m.created_at).getTime() : 0;
            return Math.abs(mTime - msgTime) <= 5000 && (m as any).__optimistic;
          });
          if (likelyIndex !== -1) {
            const newList = [...list];
  ...
  const markAsRead = useCallback(async (roomId: string) => {
    ...
    setMessages((prev) => {
      ...
      try {
        console.debug("useWsChat.markAsRead: optimistic update", { roomId, updatedCount: updated.length });
      } catch {}
  ...
  const isRoomConnected = useCallback((roomId: string) => !!roomClients.current[roomId] && roomClients.current[roomId]!.isOpen(), []);
  ...
  const addLocalMessage = useCallback((roomId: string, msg: Message) => {
    setMessages((prev) => {
      ...
      const tempId = (msg as any).__temp_id || (msg as any).tempId || (msg as any).temp_id;
      if (tempId && list.find((m) => ((m as any).__temp_id || (m as any).tempId || (m as any).temp_id) === tempId)) return prev;
