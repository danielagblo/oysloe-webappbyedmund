...
export const getUserAccountDeleteRequests = async (params?: {
  ...
  const response = await apiClient.get<AccountDeleteRequest[]>(`/api-v1/account-delete-requests/${query}`);
  return response;
};

export const createAccountDeleteRequest = async (body: { reason: string }): Promise<AccountDeleteRequest> => {
  const response = await apiClient.post<AccountDeleteRequest>(`/api-v1/account-delete-requests/`, body);
  return response;
};

export const deleteAccountDeleteRequest = async (id: number): Promise<void> => {
  await apiClient.delete<void>(`/api-v1/account-delete-requests/${id}/`);
};