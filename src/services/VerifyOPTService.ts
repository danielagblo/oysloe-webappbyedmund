...
const mockOTPs: Record<string, string> = (mockOTPsRaw as { phone: string; otp: string }[]).reduce(
  (acc, curr) => {
...
export async function sendOTP(phone: string): Promise<GenericMessage> {
  ...
  try {
    const response = await apiClient.get<GenericMessage>(endpoints.verifyOTP.send(phone));
...
export async function verifyOTP(payload: VerifyOTPRequest): Promise<GenericMessage> {
  if (useMocks) {
