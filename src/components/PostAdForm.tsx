...
import { createProductImage, deleteProductImage, getProductImages } from "../services/productImageService";
import {
  getSubcategories,
  getSubcategory,
...
// Helper: upload a File to Cloudinary (unsigned preset)
...
export default function PostAdForm({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const imgs = Array.isArray((existingProduct as any).images)
        ...
        : [];
      const mapped: UploadedImage[] = imgs
        .map((im: any, i: number) => ({
          id: i + 1,
          url:
        ...
        .filter((img: UploadedImage) => {
          // Filter out invalid URLs to avoid validation errors
          try {
            new URL(img.url);
            return true;
          } catch {
        ...
        .map((img: UploadedImage, i: number) => ({ ...img, id: i + 1 })); // Reassign ids after filtering

      // Include top-level image if present and not already in the array
      try {
  ...
  const resetForm = () => {
    ...
    setIsSubmitting(false);
    if (isMobile) setMobileStep("images");
  };
