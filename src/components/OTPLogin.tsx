...
function OTPLogin({page = "default"}: {page?: string}) {
  ...
  return (
    <Link to="/enterphone" state={{ phone: "", mode: "otp-login" }}>
      <button className="px-8 py-3 w-full bg-[#F9F9F9] max-sm:bg-white max-sm:h-[47px] max-sm:text-(--dark-def) max-sm:w-[132px] text-black rounded-full text-[12px] sm:text-xs cursor-pointer hover:bg-gray-100 transition">
        OTP Login
      </button>
    </Link>
  );
