...
const MenuBar = ({
  ...
  <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-white border border-gray-300 flex justify-around items-center gap-4 h-16 rounded-2xl shadow-lg z-10 px-6 hidden sm:flex">
    {Object.entries(icons).map(([id, icon]) => {
      ...
      const count = key === "alerts" ? alertCount : key === "inbox" ? inboxCount : undefined;
      return (
        <NavButton
          key={id}
          id={key}
...
export default function ResponsiveMenu() {
  ...
  const unreadInboxCount = chatRooms.reduce((sum, room) => sum + (room.total_unread || 0), 0);
  ...
  return isMobile ? (
    <MobileFooter active={active} onClick={handleClick} alertCount={unreadAlertCount} inboxCount={unreadInboxCount} />
  ) : (
    <MenuBar active={active} onClick={handleClick} alertCount={unreadAlertCount} inboxCount={unreadInboxCount} />
  );
}
